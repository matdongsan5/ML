...
def polyFeature(X_train, y_train, X_test, y_test, deGree=2, interactionTF = False, include_biasTF = True):
    ...
    return X_Ptrain, y_train, X_Ptest, y_test



def find_estimator(X_train, y_train, X_test, y_test, type_filter):
    """
    ...
    for (name, estimator_) in rets:
        try:
            ...
            sc2=model.score(X_test, y_test)
            result.append((name, round(sc, 2), round(sc2, 2)))
        except Exception:
            pass
