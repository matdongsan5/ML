...
class ClassfierModule:
    ...
    def knn_classification(self, n_neighbors=5):
        ...
        resultDF.loc['knn'] = [train_score, test_score, diff, 
                                train_loss, test_loss]
    ...
    def logistic_regression(self, max_iter=100):
        ...
        resultDF.loc['logi'] = [train_score, test_score, diff, 
                                train_accuracy, test_accuracy, 
                                train_classification_report, test_classification_report,
                                train_confusion_matrix, test_confusion_matrix]
    ...
    def decision_tree_classification(self,
        ...
        """
        Parameters:
        - criterion (str): 분할 품질을 측정하는 기준 ('gini' 또는 'entropy'), 기본값은 'gini'.
        - splitter (str): 분할 전략 ('best' 또는 'random'), 기본값은 'best'.
        - max_depth (int or None): 트리의 최대 깊이, 기본값은 None.
        - min_samples_split (int): 내부 노드를 분할하는 데 필요한 최소 샘플 수, 기본값은 2.
        - min_samples_leaf (int): 리프 노드가 가져야 하는 최소 샘플 수, 기본값은 1.
        - min_weight_fraction_leaf (float): 리프 노드의 최소 가중치 비율, 기본값은 0.0.
        - max_features (int, float, str or None): 최적의 분할을 위해 고려할 특성 수, 기본값은 None.
        - random_state (int or None): 난수 시드를 설정하여 결과 재현성을 확보, 기본값은 None.
        - max_leaf_nodes (int or None): 리프 노드의 최대 개수, 기본값은 None.
        - min_impurity_decrease (float): 분할을 수행할 최소 불순도 감소 값, 기본값은 0.0.
        - class_weight (dict, 'balanced' or None): 클래스 가중치 설정, 기본값은 None.
        - ccp_alpha (float): 비용-복잡성 가지치기의 매개변수, 기본값은 0.0.
        
        Returns:
        - model: 설정된 하이퍼파라미터를 가진 의사결정트리 모델
        - DF: 검증지표 DataFrame
        ...
        resultDF = pd.DataFrame(columns=['train_score', 'test_score', 'diff', 
                                        'train_loss', 'test_loss'])
        resultDF.loc['dt'] = [train_score, test_score, diff, 
                                train_loss, test_loss]
        print(classification_report(self.y_test, y_test_pred))
        return model, resultDF
    ...
    def rf_classifier(self, 
        ...
        resultDF = pd.DataFrame(columns = ['train_score', 'test_score', 'diff', 
                                        'train_loss', 'test_loss'])
        resultDF.loc['rf'] = [train_score, test_score, diff, 
                                train_loss, test_loss]
        print(classification_report(self.y_test, y_test_pred))
        return model, resultDF
    ...
    def svm_classifier(self, kernel='rbf', C=1.0, gamma='scale', random_state=42):
        """ 
        kernel='linear', 'poly', 'rbf', 'sigmoid' 선택 가능
        C: 규제(작을수록 일반화, 클수록 과적합 가능)
        gamma: 'scale' 또는 'auto'
        """
        ...
        resultDF = pd.DataFrame(columns=['train_score', 'test_score', 'diff', 'train_loss', 'test_loss'])
        resultDF.loc['svm'] = [train_score, test_score, diff, train_loss, test_loss]
        print(classification_report(self.y_test, y_test_pred))
        return model, resultDF
    
    
    def xgb_classifier(self, n_estimators=100, max_depth=6, learning_rate=0.1, random_state=42):
        ...
        resultDF = pd.DataFrame(columns=['train_score', 'test_score', 'diff', 'train_loss', 'test_loss'])
        resultDF.loc['xgb'] = [train_score, test_score, diff, train_loss, test_loss]
        print(classification_report(self.y_test, y_test_pred))
        
        return model, resultDF
    
    
    def lgbm_classifier(self, n_estimators=100, max_depth=-1, learning_rate=0.1, random_state=42):
        model = LGBMClassifier(n_estimators=n_estimators, max_depth=max_depth, learning_rate=learning_rate, random_state=random_state)
        model.fit(self.X_train, self.y_train)
        ...
        resultDF = pd.DataFrame(columns=['train_score', 'test_score', 'diff', 'train_loss', 'test_loss'])
        resultDF.loc['lgbm'] = [train_score, test_score, diff, train_loss, test_loss]
    ...
    def mlp_classifier(self, hidden_layer_sizes=(100,), activation='relu', solver='adam', max_iter=200, random_state=42):
        """ 
        인공 신경망(MLP, Multi-Layer Perceptron) 모델
        hidden_layer_sizes=(100,) → 100개의 뉴런이 있는 1개 은닉층
        activation='relu', solver='adam'이 일반적으로 좋은 선택 
        """
        
        
        model = MLPClassifier(hidden_layer_sizes=hidden_layer_sizes, activation=activation, solver=solver, max_iter=max_iter, random_state=random_state)
        model.fit(self.X_train, self.y_train)
        ...
        resultDF = pd.DataFrame(columns=['train_score', 'test_score', 'diff', 'train_loss', 'test_loss'])
        resultDF.loc['mlp'] = [train_score, test_score, diff, train_loss, test_loss]
