...
from wfCNN_f import wfCNN
model1=wfCNN()
states=torch.load(WEIGHTS_FILE, weights_only=True)
model1.load_state_dict(states)

# model2=wfCNN()
from smoke_ho import SmokeBinaryClassifier
WEIGHTS_FILE2 = MODEL_DIR+'123ho.pt'
model2=SmokeBinaryClassifier()
states2=torch.load(WEIGHTS_FILE2, weights_only=True)
model2.load_state_dict(states2)
...
model3.load_state_dict(states3)


# model4=wfCNN()
from nofireCNN_model_hye import NoFireForestCNN
# WEIGHTS_FILE4 = MODEL_DIR+'fashion_weights_epoch10_0.920_hy21.pt'
WEIGHTS_FILE4 = MODEL_DIR+'fashion_weights_epoch11_1.242hy21.pt'
model4=NoFireForestCNN()
states4=torch.load(WEIGHTS_FILE4, weights_only=True)
model4.load_state_dict(states4)
...
def pred(img, model):
        ...
        predicted = (output > 0).long().item()  # 0보다 크면 1, 작으면 0
        return predicted
...
def pre_Img(filename):
    ...
    preprocessing4 = transforms.Compose([
        transforms.Resize((50, 50)),
        transforms.ToTensor(),
        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    ])
    ...
    img2 = preprocessing2(img)
    img2 = img2.unsqueeze(0)  # 배치 차원 추가 (1, C, H, W)
    
    img3 = preprocessing3(img)
    img3 = img3.unsqueeze(0)  # 배치 차원 추가 (1, C, H, W)
    
    img4 = preprocessing4(img)
    img4 = img4.unsqueeze(0)  # 배치 차원 추가 (1, C, H, W)
        
    # except Exception as e:
    #     upload_message = f"파일 '{filename}' 이미지 로드 실패."
    #     return f"파일 '{IMG_PATH}' 이미지 로드 실패"

    pred1 = pred(img1, model1)
    pred2 = pred(img2, model2)
    pred3 = pred(img3, model3)
    pred4 = pred(img4, model4)
    pred4 = 0 if pred4 else 1
...
<body>
    ...
    <div class="upload-form">
     ...
     <div class="upload-form" >
        ...
        <form action="./caputre_cctv2.py" method="post">
            <input type="submit" value="📸 CCTV 촬영2">
        </form>
        <form action="./caputre_cctv3.py" method="post">
            <input type="submit" value="📸 CCTV 촬영3">
        </form>
        <form action="./caputre_cctv4.py" method="post">
            <input type="submit" value="📸 CCTV 촬영4">
        </form>
        <!-- 📂 업로드 폴더 정리 버튼 -->
        <form method="post">
